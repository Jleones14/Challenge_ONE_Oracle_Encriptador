...
function toggleMensaje(texto, textoEncriptado){
    if(checkString(texto) && texto != ""){
        if(mensajeEncontrado.classList.contains('aside_content_none')){
            mensajeEncontrado.classList.toggle('aside_content_none');
            ningunMensaje.classList.toggle('aside_content_none');
        }
